...
function addslidemy(imgname, imgthumb, imgtext)
{
	...
	'<div class="del"><input name="myslidedelete' + counter + '" class="myslidedelete" type="button" value="' + Joomla.JText._('COM_PANORAMA_REMOVE', 'RemoveSlide') + '" onclick="javascript:removeslide(this.getParent().getParent());" />'+
    ...
    '<a class="modal" href="' + JURI + 'administrator/index.php?option=com_media&view=images&tmpl=component&e_name=myslideimgname' + counter + '" rel="{handler:\'iframe\', size:{x: 570, y: 400}}" >' + Joomla.JText._('COM_PANORAMA_SLIDESHOWCK_SELECTIMAGE', 'select image') + '</a>'+
	'</div>'+
    '<div class="explanation"><textarea name="myslidetext' + counter + '"  class="myslidetext">'+imgtext+'</textarea></div><div style="color:red; font-weight:bold;">Total number of images MUST BE divisble and at least 4 times of column number! also precede special characters in textarea with backslash(\\)</div>');
...
function removeslide(slide) {
	if (confirm(Joomla.JText._('COM_PANORAMA_SLIDESHOWCK_REMOVE', 'Remove this slide') + ' ?')) {
		slide.destroy();
		counter--;
		storeslide();
	}
...
window.addEvent('domready', function() {
	...
	script.text = "Joomla.submitbutton = function(task){"
			+ "storeslide();"
			+ "if (task == 'panoramas.cancel' || document.formvalidator.isValid(document.id('panoramas-form'))) {	Joomla.submitform(task, document.getElementById('panoramas-form'));"
			+ "if (self != top) {"
